...
def add_components():
    ...
    if end != 0:
        prev_components.value += [solara.InputFloat(input_label.value.value[i+curr_rendered.value], value=input_value.value.value[i+curr_rendered.value], continuous_update=True) for i in range(0, end)]
...
def first_components():
    ...
    return [solara.InputFloat(input_label.value.value[i], value=input_value.value.value[i], continuous_update=True) for i in range(0, max_rendered.value if max_rendered.value < len(input_value.value.value) else len(input_value.value.value))]
