...
with open('models/x_t9_PubchemFP.json', 'r') as file:
...
import joblib
model = joblib.load('models/table_9_PubchemFP.joblib')
encoder = joblib.load('models/y_9_PubchemFP.joblib')
import pandas as pd
...
zero_value = solara.reactive(111)
...
def PubchemFP():
    DrugButton()
    solara.Text("PubchemFP (Bernoulli Naive Bayes)", style='margin-top:1rem;font-size:1.2rem;')
