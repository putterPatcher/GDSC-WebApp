...
with open('models/x_t9_SubFP.json', 'r') as file:
...
import joblib
model = joblib.load('models/table_9_SubFP.joblib')
encoder = joblib.load('models/y_9_SubFP.joblib')
import pandas as pd
...
def SubFP():
    with solara.Head():
        solara.Title("SubFP")
    DrugButton()
    solara.Text("SubFP (Bernoulli Naive Bayes)", style='margin-top:1rem;font-size:1.2rem;')
