...
with open('models/x_t9_ExtFP.json', 'r') as file:
...
import joblib
model = joblib.load('models/table_9_ExtFP.joblib')
encoder = joblib.load('models/y_9_ExtFP.joblib')
import pandas as pd
...
zero_value = solara.reactive(386)
...
def ExtFP():
    DrugButton()
    solara.Text("ExtFP (Bernoulli Naive Bayes)", style='margin-top:1rem;font-size:1.2rem;')
