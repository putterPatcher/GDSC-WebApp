...
with open('models/x_t4.json', 'r') as file:
...
import joblib
model = joblib.load('models/table_4.joblib')
encoder = joblib.load('models/y_4.joblib')
import pandas as pd
...
zero_value = solara.reactive(347)
...
@solara.component
def CopyNumberVariation():
    CellLineButton()
    solara.Text("Copy Number Variation (Bernoulli Naive Bayes)", style='margin-top:1rem;font-size:1.2rem;')
