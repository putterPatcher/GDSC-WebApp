...
import joblib
model = joblib.load('models/table_7.joblib')
encoder = joblib.load('models/y_7.joblib')
import pandas as pd
...
max_value = solara.reactive(0.999553)
min_value = solara.reactive(0.006068)
close_zero_count = solara.reactive(46)
zero_max_value = solara.reactive(0.05)
...
def fill_random_values():
    ...
    input_value.value = [random.uniform(min_value.value, max_value.value) if i not in pos else random.uniform(0.006, zero_max_value.value) for i in range(len(input_label.value))]
    disable_random.value = False

@solara.component
def AUC():
    CellLineButton()
    solara.Text("Area Under Curve (Gaussian Naive Bayes)", style='margin-top:1rem;font-size:1.2rem;')
    ...
    with solara.Div(style='text-align: center;display: flex;justify-content: space-between; align-items: center'):
        ...
        with solara.Div(style='text-align: center;display: flex;gap: 3rem; align-items: center'):
            ...
            solara.InputFloat("Min (>= 0)", value=min_value)
            solara.InputFloat("Max (<=1)", value=max_value)
